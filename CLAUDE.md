...
| ⚠ `RECOTEM_LOG_FORMAT` | auto | `auto` / `json` / `console`. An invalid value is fatal on `serve` and `inspect` only (both build a `ServeConfig`, which validates it). `train` / `validate` / `keygen` / `schema` go through `cli._configure_logging_from_env`, which is best-effort: `configure_logging` treats anything that is not `json` or `console` as `auto` and the run exits 0. |
