...
def test_min_frequency_counts_multi_label_occurrences_not_rows() -> None:
    ...
    *occurrence* count for ``multi_label``: a single row's repeated tokens
    ...
    and keep ``a`` -- pins https://recotem.org/2.1/docs/recipe-reference's documented semantics
...
def test_dimension_cap_message_matches_the_measured_scaling(
    ...
    `https://recotem.org/2.1/docs/operations` to a flat `dim^2.4`. Re-measuring across the whole
...
def test_multi_label_duplicate_tokens_encode_as_binary_not_count(
    ...
    https://recotem.org/2.1/docs/recipe-reference documents ``multi_label`` as "multi-hot"
