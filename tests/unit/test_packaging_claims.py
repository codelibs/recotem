...
assertions about ``docs/getting-started.md``'s Path B.  `#310` deleted that
file when it stopped testing documentation prose -- correctly, three of its
four tests read a markdown document -- and the fourth went with it:
...
adding ``examples`` to ``[tool.hatch.build.targets.wheel]`` left 3000 tests
green.  It is restored below, where the rest of the distribution assertions
live.  The boundary is written down because a surface half-owned by two guards
is how a surface ends up unowned -- which is what happened to this one.
...
def test_wheel_target_declares_no_examples_directory() -> None:
    ...
    anyone would actually make -- and the one the test above invites, since
    that test requires ``/examples`` in the *sdist* include list and the
    ...
    assert not named, (
        ...
        "and uninstall does not clean up; examples/ belongs in the sdist only."
    )
