...
| `storage_path` | string | `""` | Empty = in-memory (no resume). A bare path becomes a SQLite URL (`sqlite:///<path>`); explicit `sqlite://`, `postgresql+psycopg://`, and `mysql+pymysql://` URLs are also accepted. **The server-backed forms require their driver extra** — `pip install "recotem[postgres]"` for `postgresql+psycopg://`, `pip install "recotem[mysql]"` for `mysql+pymysql://`. A bare `pip install recotem` has neither, and the URL still parses without them (`sqlalchemy` arrives transitively via Optuna, `psycopg` / `pymysql` do not), so a missing extra fails at driver-import time with the same `ImportError` a wrong spelling produces. **The `+driver` suffix is required here too** — this URL goes straight to Optuna's `RDBStorage`, which has no driver preflight of its own, so a bare `postgresql://` fails inside Optuna with `ImportError: Failed to import DB access module for the specified storage URL` and a bare `postgres://` with `NoSuchModuleError`, neither naming the fix. Study name is `recotem_<recipe_name>_<run_id>` and `load_if_exists=True`, so a fresh `run_id` per train invocation always starts a new study (resume requires reusing the same `run_id` — pass `recotem train --run-id <stable>`). **SQLite over NFS corrupts** — keep SQLite databases on a local filesystem. **URLs must not embed credentials** (`postgresql://user:pass@host/db` is rejected with `SearchError` so userinfo cannot leak through SQLAlchemy tracebacks). Provide credentials via `PGPASSFILE` / `~/.pgpass` / SQLAlchemy env vars instead. |
