...
Value-side scrubbing (applied to string values of non-redacted keys):
  ...
  - 43-char base64url substrings (api key material): replaced with
    ``[REDACTED-B64URL43]``, *except* runs that are lowercase snake_case
    identifiers (see ``_SNAKE_CASE_IDENT_RE``) — those are recotem's own
    event / metric names, not credentials.
...
# Exemption for the base64url pass: a lowercase snake_case identifier is not
...
# "recipe_yaml_parse_failed_on_rescan_new_file", 43) and Prometheus metric
# names ("recotem_v1_validation_errors_outside_verb_total", 47) -- and
# redacting them destroys the only information the log line carries.
#
...
# SQLAlchemy, urllib3) through this processor, and those emit interpolated
# messages into ``event`` (observed: "Uvicorn running on http://...").  That is
# precisely the text most likely to carry an accidentally interpolated DSN or
# key, so ``event`` must keep its value-side scrubbing.  Narrowing the pattern
# fixes the false positive wherever the identifier appears, without giving up
# any coverage.
#
# Safety of the exemption: keys are generated by ``recotem keygen`` from
# ``secrets.token_urlsafe`` over the full 64-char base64url alphabet.  The
# probability that a uniformly random 43-char key lands entirely inside
# ``[a-z0-9_]`` is (37/64)**43 ~= 6e-11, and it would additionally have to
# place every underscore in separator position.  Raising the length threshold
# was rejected as an alternative: it only moves the boundary, and a 43-char
# key is exactly the shape ``keygen`` emits.
_SNAKE_CASE_IDENT_RE = re.compile(r"[a-z0-9]+(?:_[a-z0-9]+)+")


def _b64url_replacement(match: re.Match[str]) -> str:
    """Redact a base64url-shaped run unless it is a snake_case identifier."""
    run = match.group(0)
    if _SNAKE_CASE_IDENT_RE.fullmatch(run):
        return run
