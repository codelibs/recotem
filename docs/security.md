...
`configure_logging` also calls `logging.captureWarnings(True)`. Python's `warnings.warn` does not use the logging tree — the default `showwarning` writes straight to `sys.stderr` — so without this a credential a third-party library interpolates into a warning message would bypass the redaction processor entirely. Warnings are now emitted on the `py.warnings` logger, pass through the same `foreign_pre_chain`, and are rendered in the configured format like any other event.
...
**Value-side scrubbing.** Independently of the field name, string values are scanned for 64+ hex chars (`[REDACTED-HEX64]`) and 43+ base64url chars (`[REDACTED-B64URL43]`).

The base64url pass skips runs shaped like a human-authored identifier, because recotem's own event and metric names reach 43 characters (e.g. `sql_statement_timeout_unsupported_on_sqlite`) and so do filenames, object keys, business IDs and the doc anchors embedded in error messages — redacting them destroys the only information those log lines carry. A run is treated as an identifier only when all three of the following hold: it splits on `-` / `_` into two or more non-empty segments; every segment is internally case-consistent (all-lowercase, all-uppercase, or a single leading capital); and across the whole run `min(uppercase, lowercase) <= 2`, so the run is essentially single-case. That last condition is what keeps a `Title-Case-Hyphenated` secret redacted. A `keygen`-issued key is drawn uniformly from the full 64-character base64url alphabet; the probability that one satisfies all three conditions is about **1.1e-9**. `.` is outside the base64url alphabet, so dotted names are already split below the 43-character threshold and never reach this test.

**Published-digest fields.** `_HEX64_RE` exists to catch a raw 32-byte signing key rendered as hex, and cannot distinguish one from a SHA-256 digest. Two fields carry a digest that recotem publishes in the clear everywhere else and are therefore exempt from value-side scrubbing: `recipe_hash` (SHA-256 of the canonical recipe YAML, also printed by `recotem inspect` and carried in the artifact header) and `model_version` (the artifact digest, also returned in every recommend response and the `X-Recotem-Model-Version` header). The exemption is deliberately narrow: the field name must match exactly **and** the value must be nothing but the digest in the lowercase form `hexdigest()` produces, optionally prefixed with `sha256:`. Anything else in a field of that name falls through to the normal passes.
...
allocation, and a body **under** the raw cap still expands to several times its
size once it is parsed into Python objects: a ~120 MiB array-of-numbers body was
measured at ~600 MiB resident, so four concurrent such requests — all answered
`401` — pushed a serve process from ~250 MiB to ~2.7 GiB. `RECOTEM_MAX_BODY_BYTES`
bounds the **raw** bytes, not this parse expansion; bound the aggregate with a
proxy body-size limit (see the nginx block below), per-IP rate limits, and a pod
memory limit (`resources.limits.memory`, so an over-allocation OOM-kills one pod
rather than the node). The middleware enforces the cap at two
...
server {
    ...
    location /v1/ {
        ...
        # ~52 MiB) and batch verbs need more. Every increase widens the pre-auth
        # allocation, so pair a larger value with per-IP rate limits and a pod
        # memory limit.
