...
| `artifact_recipe_hash_mismatch` | WARNING. The recipe on disk has changed since this artifact was trained: the header's `recipe_hash` does not match the hash of the recipe loading it. **The artifact still loads and still serves** — this is a warning, not a refusal, because a hash difference is the expected state after any edit that does not require retraining (a comment, a rename, a serve-side `item_metadata` field). What it tells you is that the model reflects the *older* recipe, and that `/v1/recipes/{name}` is reporting that older recipe's `algorithms`, `metric` and `cutoff`. Emitted on both load paths (startup and hot-swap) with the first 12 chars of each digest. A header with **no `recipe_hash`** fails **open** (pre-2.0 artifact) and is silent. Retrain to make them agree. |
