...
- **CI now gates the tag→PyPI path — but `main` is still ungated.** Two
  ...
  green.)
- **The version must be identical everywhere it is declared**: `pyproject.toml`,
  `src/recotem/version.py`, `helm/recotem/Chart.yaml` (`version:` and
  `appVersion:`), `helm/recotem/values.yaml` (`image.tag`), and `uv.lock`. A
  mismatch between the first two nearly shipped in 2.0.0. `values.yaml` is the
  one that decides what a cluster actually pulls — `recotem.image` renders
  `.Values.image.tag | default .Chart.AppVersion` and values.yaml always sets
  it, so `appVersion` is a fallback that never fires. Do not check this by eye:
  `bash .github/scripts/check-release-tag.sh vX.Y.Z` is the authoritative check
  for every one of those except `uv.lock`, and runs locally; `uv lock --check`
  covers `uv.lock`, as does `uv sync --locked` in the pre-publish test gate.
  It has to be `--locked`:
  ...
  `uv.lock` is only ever regenerated by `uv lock` — never edited by hand.
- **A release version must be a clean PEP 440 final** (`2.0.0`), never a
  pre-release (`2.0.0a1.dev0`, `2.0.0a0`). Both release workflows now refuse one
  outright, before anything is built or pushed. Even without that guard it would
  be wrong: `pip install recotem` skips pre-releases by default, and
  `docker.yml`'s `type=semver` cannot tag one — see "The pre-release trap" below.
...
Before the guards existed, a pre-release tag published to PyPI **and pushed an
image with no version tag, while the build stayed green**. Both workflows fire
on such a tag, but `docker.yml`'s `docker/metadata-action` uses `type=semver`,
which parses SemVer only. PEP 440 and SemVer disagree on pre-releases: PEP 440
writes `2.0.0a0`, SemVer requires `2.0.0-a0`. metadata-action cannot parse the
PEP 440 form, logs `##[warning]v2.0.0a0 is not a valid semver`, and emits **no
version tags** — only `sha-<commit>`. The `Extract Docker metadata` step still
*succeeds*, so the build was green and the image went out under the sha tag
alone.
...
**Both halves of this trap are now closed.** `publish.yml`'s `guard` job and
`docker.yml`'s `guard` job run the same `.github/scripts/check-release-tag.sh`,
which refuses anything that is not `vMAJOR.MINOR.PATCH` before a single artefact
is built. `docker.yml`'s tag filter `v[0-9]+.[0-9]+.[0-9]*` is a GitHub filter
...
Run the script yourself before tagging (Phase 3, step 1) so the tag is never
created in the first place — a refused tag still has to be deleted from the
remote before the correct one can be pushed.
...
1. **Bump the version to the final `X.Y.Z`** in `pyproject.toml` and
   ...
   **`git diff uv.lock` must touch only the `recotem` entry** — unrelated
   dependency churn belongs in its own PR.
...
2. **Bump the deployment image pins** to `X.Y.Z` across `helm/`,
   ...
   It fails closed and reads every declaration together — `pyproject.toml`,
   `src/recotem/version.py`, `helm/recotem/Chart.yaml`'s `version:` and
   `appVersion:`, and `helm/recotem/values.yaml`'s `image.tag` — so it catches a
   partial bump (`pyproject.toml` moved,
...
1. **Pin and verify the merge commit.** Do not rely on `main` staying put —
   ...
   git show "${SHA}:helm/recotem/Chart.yaml" | grep -E '^(app)?[Vv]ersion'  # X.Y.Z
   git show "${SHA}:helm/recotem/values.yaml" | grep -A2 '^image:'  # tag: "X.Y.Z"

   # Authoritative — the same check both `guard` jobs run at the tag.
   ...
   The `git show` lines read the commit; the script reads the **working
   ...
   partial bump, which reading greps by eye does not.
...
7. **Verify both registries actually received the version.** The two workflows
   run independently and only `publish.yml` waits on the `pypi` environment
   approval, so one registry can have the version while the other does not.
   Confirm both rather than reading it off the run list:
...
| Tag pushed, no GHCR **version** tag | `docker.yml` is red — at *release tag guard* (the tag is not `vMAJOR.MINOR.PATCH`, or disagrees with the tree) or at `trivy`. Either way nothing was pushed; a lone `sha-` tag on GHCR is from an earlier branch push. |
...
| `publish` red at *release tag guard* | The tag is not `vMAJOR.MINOR.PATCH`, or disagrees with `pyproject.toml` / `version.py` / `helm/recotem/Chart.yaml` / `helm/recotem/values.yaml`. Nothing was built, and `docker` is red at its own guard for the same reason. Delete the tag, fix the tree, re-tag. Run `check-release-tag.sh` first next time. |
