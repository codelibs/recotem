...
| `storage_path` | string | `""` | Empty = in-memory (no resume). A bare path becomes a SQLite URL (`sqlite:///<path>`); explicit `sqlite://`, `postgresql+psycopg://`, `mysql+pymysql://`, and `mariadb+pymysql://` URLs are also accepted. **The `+driver` suffix is required here too.** A value that names an unsupported dialect, a removed dialect (`postgres://`), or a driver that is not installed on this host is refused up front with **exit 8** (`storage_path_unusable`), naming the spelling to use and the extra that provides the driver (`recotem[postgres]` for psycopg, `recotem[mysql]` for PyMySQL — which serves both the `mysql` and `mariadb` dialects; a bare `pip install recotem` has neither) — `recotem validate` performs the same check, so it is caught before any data is fetched rather than mid-training after a scan has been billed. Note that an unrecognised URL scheme is **not** treated as a filename: `oracle://host/db` is refused, not opened as a SQLite file called `oracle://host/db`. Whether the backend actually *opens* cannot be decided from the recipe text, so it is not part of that pre-flight — a missing or unwritable SQLite directory, or a server that is down or refuses the connection, is caught when the study is built and also reports **exit 8** naming `training.storage_path`, rather than the unmapped exit 1 an Optuna-internal `OperationalError` used to produce. Study name is `recotem_<recipe_name>_<run_id>` and `load_if_exists=True`, so a fresh `run_id` per train invocation always starts a new study (resume requires reusing the same `run_id` — pass `recotem train --run-id <stable>`). **SQLite over NFS corrupts** — keep SQLite databases on a local filesystem. **URLs must not embed userinfo**, and that includes a bare username — `postgresql+psycopg://recotem@host/db` is refused just as `postgresql+psycopg://recotem:pass@host/db` is, because credentials in a study URL end up in SQLAlchemy exception traces that the log redaction processor cannot reach (it redacts by dict key). `recotem validate` performs this check too, with **exit 8**, so it is caught before any data is fetched. For `postgresql`, supply both parts from the environment: `PGUSER` for the user, `PGPASSFILE` / `~/.pgpass` for the password, and write the URL as `postgresql+psycopg://host:port/dbname`. For `mysql` / `mariadb` there is no equivalent: **pymysql reads no user or password environment variable**, so a MySQL or MariaDB study backend must accept the OS account the training process runs as. If it cannot, use a PostgreSQL study backend or a local SQLite path. |
