...
- **Authors**: shinsuke
- **Related**:
  - downstream pilot (external, not tracked in this repo): smartstocknotes.jp playground that drove the v1 design.
...
(Survey conducted via direct fetch of vendor docs; key citations: AWS Personalize Runtime API reference, Google Vertex AI Search for Retail Predict API, Algolia Recommend REST API, Microsoft Azure AI Personalizer, Spotify Web API. URLs in the design-session notes.) 主要レコメンドAPIから抽出した業界共通パターン:
...
- [ ] OpenAPI スキーマ (`/openapi.json`) が 9 エンドポイントを正しく公開。
