...
|---|---|---|
| `DATABASE_URL` | `postgresql://recotem_user:recotem_pass@localhost:5432/recotem` | Read-only PostgreSQL connection |
...
The inference service uses SQLAlchemy with read-only access to Django's PostgreSQL database. SQLAlchemy models mirror the Django schema:
...
POST /inference/predict/project/{project_id}
  ...
  |
  +-- 6. Return response with slot_id and slot_name
...
- Database connections are read-only (no write contention)
